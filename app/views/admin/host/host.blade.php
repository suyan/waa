    ...
    @foreach ($hosts as $host)
      <tr id="host_{{ $host->id }}">
        ...
        <td class="status_{{ $host->id }}_0 {{ $host->status == 0 ?: 'hide' }}">
          <a href="{{ URL::to('admin/host/'.$host->id.'/run') }}" class="btn btn-primary btn-xs">@lang('host.start')</a>
          <a href="{{ URL::to('admin/host/'.$host->id.'/delete') }}" class="btn btn-danger btn-xs">@lang('host.delete')</a>
        </td>
        ...
        <td class="status_{{ $host->id }}_3 {{ $host->status == 3 ?: 'hide' }}">
          <a href="{{ URL::to('admin/host/'.$host->id.'/info') }}" class="btn btn-default btn-xs">@lang('host.info')</a>
          <a href="{{ URL::to('admin/host/'.$host->id.'/delete') }}" class="btn btn-danger btn-xs">@lang('host.delete')</a>
        </td>
        <td class="status_{{ $host->id }}_4 {{ $host->status == 4 ?: 'hide' }}">@lang('host.run_error')</td> 
        <td class="status_{{ $host->id }}_4 {{ $host->status == 4 ?: 'hide' }}">
          <a href="{{ URL::to('admin/host/'.$host->id.'/run') }}" class="btn btn-primary btn-xs">@lang('host.restart')</a>
          <a href="{{ URL::to('admin/host/'.$host->id.'/delete') }}" class="btn btn-danger btn-xs">@lang('host.delete')</a>
        </td>
